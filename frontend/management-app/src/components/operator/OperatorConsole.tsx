...
export const OperatorConsole: React.FC = () => {
  ...
  const [centres] = useState<any[]>([{ id: 'centre-samrala', name: 'Loading...', code: '...', yardCapacityPercent: 0 }]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch all bookings for queue
        const bookingsRes = await axios.get(`${baseURL}/bookings`);
        if (bookingsRes.data && bookingsRes.data.data) {
          const items = Array.isArray(bookingsRes.data.data) ? bookingsRes.data.data : bookingsRes.data.data.items || [];
          const mappedTokens = items.map((b: any) => ({
            id: b.id,
            ...
            vehicleNumber: 'PB-XX-1234', // Mocked until schema updated
            vehicleType: 'Tractor Trolley',
            cropName: b.crop?.name || 'Wheat',
            estimatedQuintals: b.quantity || 50,
            status: b.status === 'PENDING' ? 'BOOKED' : (b.status === 'CONFIRMED' ? 'GATE_VERIFIED' : b.status),
          ...
          if (mappedTokens.length > 0) {
            setSelectedCentreId(mappedTokens[0].centreId);
          }
        }

        // Fetch dashboard stats (just passing arbitrary centre_id for now)
        // In real app, we get the logged in operator's centre
      } catch (err) {
                        ...
                        {tok.status === 'COMPLETED' && (
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition flex items-center space-x-1 cursor-pointer"
