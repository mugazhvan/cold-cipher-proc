...
export const DigitalTokenPassModal: React.FC<DigitalTokenPassModalProps> = ({
  ...
  const modalQrCanvasRef = useRef<HTMLCanvasElement | null>(null);

  const qrPayload =
    token.qrCodeValue ||
    `KISANFLOW://TOKEN/${token.tokenNumber}/${token.vehicleNumber}`;

  useEffect(() => {
    if (isOpen && modalQrCanvasRef.current) {
      QRCode.toCanvas(modalQrCanvasRef.current, qrPayload, {
        width: 176,
        margin: 1,
        color: {
          dark: '#020617',
        ...
        console.error('Failed to render modal QR code:', err);
      });
    }
  }, [isOpen, qrPayload]);
  ...
  const generatePassCanvas = async (): Promise<HTMLCanvasElement> => {
    ...
    // Generate Real Scannable 2D QR Image for Canvas
    const qrStartX = 85;
    const qrStartY = 715;
    const qrSize = 190;
    ...
    try {
      ...
      ctx.drawImage(qrImg, qrStartX, qrStartY, qrSize, qrSize);
    } catch (e) {
      console.error('Failed drawing QR to download canvas:', e);
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(qrStartX, qrStartY, qrSize, qrSize);
    }

    // QR Text & Manual Fallback details on the right
    ctx.textAlign = 'left';
    ctx.fillStyle = '#34d399';
    ctx.font = 'bold 18px "Segoe UI", Roboto, sans-serif';
    ctx.fillText('MANDI GATE ADMISSION QR CODE', 300, 740);

    ctx.fillStyle = '#94a3b8';
    ctx.font = '13px "Segoe UI", Roboto, sans-serif';
    ctx.fillText('Scan at Mandi Security Gate 1 for automatic vehicle admission,', 300, 768);
    ctx.fillText('gross weight weighbridge queuing, and lane allocation.', 300, 788);

    // Manual Fallback Box inside Downloaded Pass
    ctx.fillStyle = '#1e293b';
    ctx.fillRect(300, 805, 415, 60);
    ctx.strokeStyle = '#f59e0b';
    ctx.lineWidth = 1.5;
    ctx.strokeRect(300, 805, 415, 60);

    ctx.fillStyle = '#f59e0b';
    ctx.font = 'bold 11px "Segoe UI", Roboto, sans-serif';
    ctx.fillText('MANUAL GATE ENTRY TOKEN (FALLBACK):', 315, 825);

    ctx.fillStyle = '#fde68a';
    ctx.font = 'bold 22px monospace';
    ctx.fillText(token.tokenNumber, 315, 852);

    ctx.fillStyle = '#a7f3d0';
    ctx.font = '12px "Segoe UI", Roboto, sans-serif';
    ctx.fillText('• MSP Guaranteed Direct Benefit Transfer (DBT)', 300, 892);
    ctx.fillText('• 100% Free Government Transit & Moisture Testing', 300, 912);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `KisanFlow_ePass_${token.tokenNumber}.pdf`);
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-3 space-y-1.5">
                  ...
                  <div className="flex items-center justify-between bg-slate-900 p-2 rounded-lg border border-slate-800">
                    <span className="font-mono text-sm font-black text-amber-300 select-all">
                      {token.tokenNumber}
                    </span>
                    <button
                      type="button"
                      onClick={handleCopyToken}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-lg transition flex items-center space-x-1 cursor-pointer"
                      title="Copy Token"
                    >
                      {copied ? (
                        <>
        ...
        <div className="bg-slate-950 px-6 py-4 border-t border-slate-800 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span>{downloading ? 'Generating...' : 'Download Official PDF'}</span>
            </button>
