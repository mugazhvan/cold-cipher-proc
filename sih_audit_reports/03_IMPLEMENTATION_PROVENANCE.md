# KisanFlow: Implementation vs. Proposal Audit

This document precisely classifies major project features into four categories based on the actual codebase forensics: **IMPLEMENTED**, **SIMULATED**, **PROPOSED**, and **RESEARCHED ONLY**.
...
| **Wait-time Estimation** | IMPLEMENTED (Heuristic) | `predictions.py` (Utilization math) | "Deterministic, capacity-aware utilization logic estimates active yard congestion." |
| **Document Generation (J-Form)** | IMPLEMENTED (PDF Prototype) | `pdf_generator.py` | "Automates standard digital Form 'J' procurement receipts instantly." |
...
| **e-NAM / AGMARKNET Integration** | SIMULATED (Domain Model) | N/A (Schema only) | "Features a schema designed for seamless interoperability with e-NAM." |
| **UIDAI (Aadhaar) / PFMS** | SIMULATED (Domain Model) | N/A (Schema only) | "Data architecture is structured to support future PFMS/UIDAI hooks." |
...
| **AI / ML Recommendations** | NOT FOUND | `predictions.py` contains no ML logic | *(Do not claim AI/ML. Claim "Algorithm" or "Heuristic" instead)* |
